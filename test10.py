...
for i in range(7):
    ...
    fff = open(f'{i+13}_12_2020__ww.txt', 'w', encoding='utf-8')
    all_cur = 0
    all_money = 0
    all_s = 0
    all_text = ""
    ...
    for uid in UIDXX:
        ...
        try:
            all_text += f"\n{i+13}.12.2020  @{db_cmd.get_user_data(uid)[1]}\n\n"
        except Exception as e:
            print(uid)
            all_text += f"\n{i+13}.12.2020  user_id:{uid}\n\n"
        for elx in list_xxx:
    ...
    fff.write(all_text)
    fff.close()
    fff = open(f'{i+13}_12_2020__ww.txt', 'rb')
    bot.send_document(871500685, fff, caption="askme")
    fff.close()
